...
public:
    ...
    [[maybe_unused]] static void create_weights_memory(cldnn::engine& engine, cldnn::memory::ptr base, moe_weights& pw, 
        ...
        auto weight_type = op->get_input_element_type((size_t)MOEInputIndex::WEIGHT_0);
        auto zp_type     = op->get_input_element_type((size_t)MOEInputIndex::ZP_0);
        auto scale_type  = op->get_input_element_type((size_t)MOEInputIndex::SCALE_0);
    ...
    static void fill_weights_memory(
        ...
        fill(get_constant_input((size_t)MOEInputIndex::WEIGHT_0), wei_mem.gate_w);
        fill(get_constant_input((size_t)MOEInputIndex::ZP_0), wei_mem.gate_z);
        fill(get_constant_input((size_t)MOEInputIndex::SCALE_0), wei_mem.gate_s);

        fill(get_constant_input((size_t)MOEInputIndex::WEIGHT_1), wei_mem.up_w);
        fill(get_constant_input((size_t)MOEInputIndex::ZP_1), wei_mem.up_z);
        fill(get_constant_input((size_t)MOEInputIndex::SCALE_1), wei_mem.up_s);

        fill(get_constant_input((size_t)MOEInputIndex::WEIGHT_2), wei_mem.down_w);
        fill(get_constant_input((size_t)MOEInputIndex::SCALE_2), wei_mem.down_s);
        fill(get_constant_input((size_t)MOEInputIndex::ZP_2), wei_mem.down_z);
    }
    ...
    static void fill_weights_memory_from_disk_v2(cldnn::engine& engine, const std::shared_ptr<MOE3GemmFusedCompressed>& op, 
        ...
        for (uint32_t expert: experts_list) {
            fill_from_disk(get_constant_input((size_t)MOEInputIndex::WEIGHT_0),
               wei_mem.gate_w, expert, lru_experts[i]);

            fill_from_disk(get_constant_input((size_t)MOEInputIndex::WEIGHT_1),
                           wei_mem.up_w, expert, lru_experts[i]);

            fill_from_disk(get_constant_input((size_t)MOEInputIndex::WEIGHT_2),
                           wei_mem.down_w, expert, lru_experts[i]);

            fill_from_disk(get_constant_input((size_t)MOEInputIndex::SCALE_0),
                           wei_mem.gate_s, expert, lru_experts[i]);

            fill_from_disk(get_constant_input((size_t)MOEInputIndex::ZP_0),
                           wei_mem.gate_z, expert, lru_experts[i]);

            fill_from_disk(get_constant_input((size_t)MOEInputIndex::SCALE_1),
                           wei_mem.up_s, expert, lru_experts[i]);

            fill_from_disk(get_constant_input((size_t)MOEInputIndex::ZP_1),
                           wei_mem.up_z, expert, lru_experts[i]);

            fill_from_disk(get_constant_input((size_t)MOEInputIndex::SCALE_2),
                           wei_mem.down_s, expert, lru_experts[i]);

            fill_from_disk(get_constant_input((size_t)MOEInputIndex::ZP_2),
