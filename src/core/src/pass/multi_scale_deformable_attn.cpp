...
#include "openvino/pass/multi_scale_deformable_attn.hpp"
...
ov::pass::MultiScaleDeformableAttn::MultiScaleDeformableAttn() {}
...
bool ov::pass::MultiScaleDeformableAttn::run_on_model(const std::shared_ptr<ov::Model>& model) {
    RUN_ON_MODEL_SCOPE(MultiScaleDeformableAttn);
    ...
    //                 "the MultiScaleDeformableAttn transformation.");
        ...
        if (auto param = get_parameter(model, param_name)) {
            ...
            if (param->output(0).get_target_inputs().size() == 0) {
                ...
                OPENVINO_ASSERT(param->output(0).get_target_inputs().size() == 0,
                                "MSDA transformation failed: couldn't remove ",
            ...
            std::cout << "=========================== SDPA_TO_MSDA (" << model->get_friendly_name() << ") success! =====================" << std::endl;
            ...
            for (auto target : param->get_output_target_inputs(0)) {
                ...
                std::shared_ptr<op::internal::MSDA> msda;
                msda = std::make_shared<op::internal::MSDA>(inputs);
                msda->set_friendly_name(sdpa->get_friendly_name());

                for (auto& consumer : sdpa_consumers)
                    consumer.replace_source_output(msda);
            }
        }
    }
