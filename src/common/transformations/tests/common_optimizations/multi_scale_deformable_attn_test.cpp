...
std::shared_ptr<ov::Node> build_grid_sample_block(const std::shared_ptr<ov::Node>& attn_Reshape, const std::shared_ptr<ov::Node>& attn_Sub,
    ...
    auto attn_Slice = std::make_shared<StridedSlice>(attn_Reshape, Constant_124164, Constant_125832, Constant_124170, std::vector<int64_t>{1,0}, std::vector<int64_t>{1,0});
    auto attn_Reshape_4 = std::make_shared<Reshape>(attn_Slice, Constant::create(element::i64, Shape{3}, {0,0,256}), true);
    auto attn_Transpose = std::make_shared<Transpose>(attn_Reshape_4, Constant::create(element::i64, Shape{3}, {0,2,1}));
    // [?,256,16700] [?,256,4200] [?,256,1050] [?,256,273] =>
    // {-1,32,100,167}  {-1,32,50,84} {-1,32,25,42} {-1,32,13,21}
    auto attn_Reshape_5 = std::make_shared<Reshape>(attn_Transpose, Constant::create(element::i64, Shape{scale_size.size()}, scale_size), true);
    
    // [1,22223,8,1,4,2] ?? [1,22223,8,4,2] ??
    auto attn_Gather_9 = std::make_shared<Gather>(attn_Sub, Constant::create(element::i64, Shape{sub_indices.size()}, sub_indices), Constant::create(element::i64, Shape{sub_axis.size()}, sub_axis), 0);
    auto attn_squeeze = std::make_shared<Squeeze>(attn_Gather_9, Constant::create(element::i64, Shape{1}, sub_axis));  // FIXME???
    auto attn_Transpose_1 = std::make_shared<Transpose>(attn_squeeze, Constant::create(element::i64, Shape{5}, {0,2,1,3,4}));
    auto attn_Reshape_6 = std::make_shared<Reshape>(attn_Transpose_1, Constant::create(element::i64, Shape{4},{-1,22223,4,2}), true);
    GridSample::Attributes attributes = {false, GridSample::InterpolationMode::BILINEAR, GridSample::PaddingMode::ZEROS};
    auto attn_GridSample = std::make_shared<GridSample>(attn_Reshape_5, attn_Reshape_6, attributes);
    auto attn_Unsqueeze_31 = std::make_shared<Reshape>(attn_GridSample, Constant::create(element::i64, Shape{5}, {-1,32,22223,1,4}), false);
...
std::shared_ptr<ov::Node> build_concated_grid_samplers(const std::shared_ptr<ov::Node>& attn_Reshape, const std::shared_ptr<ov::Node>& attn_Sub) {
    ...
    //0
    auto attn_Unsqueeze_31 = build_grid_sample_block(attn_Reshape, attn_Sub, {0,0}, {0,16700}, {batch_size * 8,32,100,167}, {0}, {3});

    //1
    auto attn_Unsqueeze_32 = build_grid_sample_block(attn_Reshape, attn_Sub, {0,16700}, {0,20900}, {batch_size * 8,32,50,84}, {1}, {3});

    //2
    auto attn_Unsqueeze_33 = build_grid_sample_block(attn_Reshape, attn_Sub, {0,20900}, {0,21950}, {batch_size * 8,32,25,42}, {2}, {3});

    //3
    auto attn_Unsqueeze_34 = build_grid_sample_block(attn_Reshape, attn_Sub, {0,21950}, {0,22223}, {batch_size * 8,32,13,21}, {3}, {3});

    auto attn_Concat_17 = std::make_shared<Concat>(ov::NodeVector{attn_Unsqueeze_31, attn_Unsqueeze_32, attn_Unsqueeze_33, attn_Unsqueeze_34}, -2);   //  tensor_array<f16[?,32,22223,4,4]> /encoder/layers.1/self_attn/Concat_17(/encoder/layers.1/self_attn/Unsqueeze_31, /encoder/layers.1/self_attn/Unsqueeze_32, /encoder/layers.1/self_attn/Unsqueeze_33, /encoder/layers.1/self_attn/Unsqueeze_34)
    auto attn_Reshape_17 = std::make_shared<Reshape>(attn_Concat_17, Constant::create(element::i64, Shape{4}, {0,32,0,16}), true);   //  tensor_array<f16[?,32,22223,16]> /encoder/layers.1/self_attn/Reshape_17(/encoder/layers.1/self_attn/Concat_17, Constant_650341)
...
std::shared_ptr<ov::Node> build_attn_aggregate(const std::shared_ptr<ov::Node>& input_attn_weight, const std::shared_ptr<ov::Node>& grid_sample) {
    ...
    auto attn_Reshape_18 = std::make_shared<Reshape>(attn_ReduceSum, Constant::create(element::i64, Shape{3}, {-1,256,0}), true);   //  tensor_array<f16[?,256,22223]> /encoder/layers.1/self_attn/Reshape_18(/encoder/layers.1/self_attn/ReduceSum, Constant_650345)
    auto attn_output_proj_MatMul_transpose_a = std::make_shared<Transpose>(attn_Reshape_18, Constant::create(element::i64, Shape{3}, {0,2,1}));   //  tensor_array<f16[?,22223,256]> /encoder/layers.1/self_attn/output_proj/MatMul/transpose_a(/encoder/layers.1/self_attn/Reshape_18, Constant_48800)
